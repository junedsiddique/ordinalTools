...
#' Calculate means from an ordinal model
#'
#' @param object An object of class "polr"
#' @param data A data frame. Only required when se.type="bootstrap"
...
#' * se.type Method used to calculate standard erros
#' @export
#'
#' @examples
#' \dontrun{
...
#' fit <- ordinal_means(polr.fit, contrast1=c(0,0,0), contrast2=c(0,0,0))
#' summary(fit)
#'
#' # Two coefficient constrasts and their difference
#' # Bootstrap standard errors
#' fit <- ordinal_means(polr.fit, data=data, contrast1=c(0,0,0),
#'        contrast2=c(0,0,0), type="bootstrap")
#' summary(fit)
#' }
#' @details This function uses the parameter estimates from an proportional odds
#' logistic regression model that was fit using the MASS::polr function to obtain
...
ordinal_means <- function(object, data=NULL, conf.level = 0.95,
...
# Error handling
if (length(contrast1) != length(coef(object))){
  stop("Length of contrast 1 vector should be equal to ",
       length(coef(object)), " which is the length of the coefficient vector.")
}

if (!is.null(contrast2) & length(contrast2) != length(coef(object))){
    stop("Length of contrast 1 vector should be equal to ",
         length(coef(object)), " which is the length of the coefficient vector.")
}

if (se.type=="bootstrap" & is.null(data)){
    stop("Data set must be specified in order to use se.type='bootstrap'")
  }
